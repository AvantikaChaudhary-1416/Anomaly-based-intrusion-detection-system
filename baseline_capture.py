...
import ipaddress
import os
import glob as glob_module   # aliased — same shadowing issue as datetime: scapy's
                              # wildcard import below also exports something named `glob`
import datetime as dt_module   # aliased — scapy's `from scapy.all import *` below also
                                # pulls in a `datetime` class that would shadow the module
import json
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.isdir(target):
            ...
            if not pcap_files:
                ...
                # Different pcaps were captured on different networks
                # (college / home / etc), so a single global my_host is
                # wrong for a multi-file run. inferred_hosts.json (from
                # infer_my_host.py) maps filename -> ip. We swap my_host
                # right before each file's packets are read, while still
                # accumulating window_counts/per_ip_obs/flows/ip_summary
                # across ALL files into one combined baseline.
                host_map = {}
