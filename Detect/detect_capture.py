...
# separate filenames — never overwrite training data in Baseline/
...
def update_flow(pkt):
    ...
    if key not in flows:
        flows[key] = {
            'initiator': src_ep, 'responder': dst_ep,
            'proto': pkt[IP].proto,
            'start': now_t, 'last': now_t,
...
print("=" * 55)
print("  Detection Capture (feeds trained models — no baseline/EMA output)")
print("=" * 55)
...
print(f"  Outputs:   {FLOWS_FILE}, {WINDOWS_FILE}, {IP_WINDOWS_FILE}")
print(f"\n  Starting capture...\n")
